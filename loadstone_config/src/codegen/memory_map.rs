...
pub fn generate<P: AsRef<Path>>(
    ...
    validate_bank_count(memory_configuration)?;
...
fn validate_bank_count(configuration: &MemoryConfiguration) -> Result<()> {
    let count = configuration.internal_memory_map.banks.len()
        + configuration.external_memory_map.banks.len();
    if count <= (u8::MAX as usize) {
        Ok(())
    } else {
        Err(anyhow::format_err!(
            "A bank count greater than {} is not supported (found {}).",
            u8::MAX,
            count
        ))
    }
...
fn generate_mcu_banks(
    ...
    let size: Vec<usize> = map
        ...
        .map(|b| (b.size_kb * 1024) as usize)
        .collect();
    let golden: Vec<bool> = (0..number_of_mcu_banks)
        .map(|i| Some(i) == golden_index)
        .collect();
