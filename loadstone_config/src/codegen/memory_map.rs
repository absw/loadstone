...
fn validate_bank_count(configuration: &MemoryConfiguration) -> Result<()> {
    ...
    if count <= (u8::MAX as usize) {
        ...
        Err(anyhow::format_err!(
            "A bank count greater than {} is not supported (found {}).", u8::MAX, count))
    }
}
...
fn generate_imports(memory_configuration: &MemoryConfiguration, port: &Port) -> Result<String> {
    let external_address: Vec<_> = match &memory_configuration.external_flash {
        ...
        Some(external_flash) if external_flash.name.to_lowercase().contains("is25lp128f") => {
            ["blue_hal", "drivers", "is25lp128f", "Address"]
                .iter()
                .map(|f| format_ident!("{}", f))
                .collect()
        }
