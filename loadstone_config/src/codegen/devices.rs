...
use crate::{codegen::prettify_file, features::{Serial, UpdateSignal}, Configuration};
...
pub fn generate<P: AsRef<Path>>(
    ...
    match configuration.port {
        crate::port::Port::Stm32F412 => {
            ...
            generate_update_signal_stm32(configuration, &mut code)?;
        }
...
fn generate_update_signal_stm32(
    configuration: &Configuration,
    code: &mut quote::__private::TokenStream,
) -> Result<()> {
    let enabled = matches!(configuration.feature_configuration.update_signal, UpdateSignal::Enabled);
    if enabled {
        code.append_all(quote! {
            use blue_hal::drivers::stm32f4::update_signal::FixedUpdateSignal;

            #[allow(unused)]
            pub type UpdateSignalType = FixedUpdateSignal;

            #[allow(unused)]
            pub fn construct_update_signal() -> UpdateSignalType {
                FixedUpdateSignal::new(false)
            }
        })
    } else {
        code.append_all(quote! {
            ...
            #[allow(unused)]
            pub type UpdateSignalType = NullUpdateSignal;
